...
> **新增：QQ 临时会话拦截**
> 支持自动拦截通过 QQ 群发起的临时会话，防止用户绕过群聊封禁直接私聊 bot。仅对 aiocqhttp（QQ 个人号）平台有效，其他平台不受影响。
...
> 仅对 **aiocqhttp（QQ 个人号）** 平台有效。

QQ 用户可通过群聊右键向 bot 发起「临时会话」私聊。开启临时会话拦截后，**所有临时会话均会被拦截，无论发送者是否在封禁名单中**，消息在进入任何处理前就被直接阻断。

在插件配置中可调整以下选项：

| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `block_temp_msg` | bool | `true` | 是否拦截临时会话 |
| `temp_msg_reply` | str | `"请勿通过临时会话联系我，请在群内发言。"` | 拦截时发送的提示语，留空则静默拦截 |

> ⚠️ **注意**：关闭 `block_temp_msg` 后，群维度封禁（`/ban`）对临时会话**不生效**，仅全局封禁（`/ban-all`）仍有效。
