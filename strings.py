...
messages = {
    ...
    "ban_umo": "已禁用会话 {umo}，时限：{time}，理由：{reason}",
    "pass_umo": "已临时解限会话 {umo}，时限：{time}，理由：{reason}",
    "dec_ban_umo": "已删除对会话 {umo} 的禁用（{time}），理由：{reason}",
    "dec_pass_umo": "已删除对会话 {umo} 的临时解限（{time}），理由：{reason}",
