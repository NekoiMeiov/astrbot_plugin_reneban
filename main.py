...
class ReNeBan(Star):
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        self.messages = {
            ...
            "passed_user_with_reason": "已在全局临时解限 {user}，时限：{time}，理由：{reason}",
...
🚫 限制命令：
{self.commands['ban']} - 在当前群限制用户（若群内已存在限制，则叠加）
{self.commands['ban-all']} - 全局限制用户（若全局已存在限制，则叠加）

🎀 解限命令：
{self.commands['pass']} - 解除当前群限制（允许临时解限，若已有解除时限，则叠加）
...
- 过期限制会自动清理"""
    ...
    async def pass_all(self, event: AstrMessageEvent, passuser: str, time: str = "0", reason: str | None = None, end: str | None = None):
        ...
        if time == "0":
            ...
            yield event.plain_result(self.messages["passed_user_with_reason"].format(user=pass_uid, time=self.time_format(time), reason=reason))
